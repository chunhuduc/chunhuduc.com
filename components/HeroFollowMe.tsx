...
const linkClassName =
  "inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-accent/50 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent";
...
function IconGitHub() {
  /* Simple Icons silhouette: ears + tail read clearly at small sizes */
  return (
...
/**
 * Flat monochrome contact icons row (referencing muted row + bright hover glyph).
 * Styling hooks: `.contact-social-links` in `globals.css` (sibling fade, slide-up hover).
 */
export default function HeroFollowMe({
  ...
  const zaloUrl = (typeof s.zalo === "string" && s.zalo.trim()) || `https://zalo.me/${digits}`;
  ...
  return (
    <div className={className}>
      ...
      <div className={`contact-social-links mt-5 ${rowToneClass}`}>
        {gh ? (
        ...
        {li ? (
          <FollowChip href={li} label="LinkedIn">
            <IconLinkedIn />
          </FollowChip>
        ) : null}
        {uw ? (
          <FollowChip href={uw} label="Upwork">
            <Image
        ...
        {fb ? (
          <FollowChip href={fb} label="Facebook">
            <IconFacebook />
          </FollowChip>
        ) : null}
        {dc ? (
          <FollowChip href={dc} label="Discord">
            <IconDiscord />
          </FollowChip>
        ) : null}
        {tg ? (
          <FollowChip href={tg} label="Telegram">
            <IconTelegram />
          </FollowChip>
        ) : null}

        <FollowChip href={waUrl} label="WhatsApp">
          <IconWhatsApp />
        </FollowChip>

        {/* <FollowChip href={zaloUrl} label="Zalo">
          <Image
            src="/icons/zalo.svg"
            alt=""
            width={24}
        ...
        <FollowChip href={`tel:${digits}`} label="Phone">
          <IconPhone />
        </FollowChip>
