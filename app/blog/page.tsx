    ...
    <div className="mx-auto max-w-5xl px-4 py-14 sm:px-6 sm:py-20">
      <header className="max-w-3xl">
        ...
        <p className="mt-3 text-base leading-relaxed text-muted">
          ...
          <code className="rounded bg-line/60 px-1 font-mono text-sm">content/blog</code>.
        </p>
      </header>

      <ul className="mt-12 divide-y divide-line border border-line rounded-xl bg-white/40">
        {posts.map((post) => (
          <li key={post.slug}>
            <Link
              href={`/blog/${post.slug}`}
